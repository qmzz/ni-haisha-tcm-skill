...
REPORT_PATH = ROOT / "report" / "p54_ascii_dot_tail_cleanup.md"
...
ASCII_DOT_TAIL_RE = re.compile(r"[ \t]*\.{10,}(?=(?:[\"\r\n]|$))")


def clean_text(text: str) -> tuple[str, int]:
    count = len(ASCII_DOT_TAIL_RE.findall(text))
    return ASCII_DOT_TAIL_RE.sub("", text), count
...
def main() -> None:
    ...
    lines = [
        ...
        f"- ASCII dot tails removed: {total_removed}",
        "",
        "## Markdown Changes",
    ]
    for rel, count in markdown_changes:
        lines.append(f"- `{rel}`: {count} ASCII dot tails")

    lines.extend(["", "## JSONL Changes"])
    for rel, rows, count in jsonl_changes:
        lines.append(f"- `{rel}`: {rows} rows, {count} ASCII dot tails")
