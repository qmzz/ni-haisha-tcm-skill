...
REPORT_PATH = ROOT / "report" / "p51_toc_dot_leader_cleanup.md"
...
TOC_DOT_LEADER_RE = re.compile(r"[ \t]*\u00b7{6,}[ \t]*[0-9]{1,4}(?:[ \t]*,[ \t]*[0-9]{1,4})*")


def clean_text(text: str) -> tuple[str, int]:
    count = len(TOC_DOT_LEADER_RE.findall(text))
    return TOC_DOT_LEADER_RE.sub("", text), count
...
def main() -> None:
    ...
    lines = [
        ...
        f"- Dot leader page markers removed: {total_removed}",
        "",
        "## Markdown Changes",
    ]
    for rel, count in markdown_changes:
        lines.append(f"- `{rel}`: {count} dot leader page markers")

    lines.extend(["", "## JSONL Changes"])
    for rel, rows, count in jsonl_changes:
        lines.append(f"- `{rel}`: {rows} rows, {count} dot leader page markers")
