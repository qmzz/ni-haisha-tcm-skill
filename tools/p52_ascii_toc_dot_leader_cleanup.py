...
REPORT_PATH = ROOT / "report" / "p52_ascii_toc_dot_leader_cleanup.md"
...
ASCII_TOC_DOT_LEADER_RE = re.compile(r"[ \t]*\.{6,}[ \t]*[0-9]{1,4}(?:[ \t]*,[ \t]*[0-9]{1,4})*")


def clean_text(text: str) -> tuple[str, int]:
    count = len(ASCII_TOC_DOT_LEADER_RE.findall(text))
    return ASCII_TOC_DOT_LEADER_RE.sub("", text), count
...
def main() -> None:
    ...
    lines = [
        ...
        f"- ASCII dot leader page markers removed: {total_removed}",
        "",
        "## Markdown Changes",
    ]
    for rel, count in markdown_changes:
        lines.append(f"- `{rel}`: {count} ASCII dot leader page markers")

    lines.extend(["", "## JSONL Changes"])
    for rel, rows, count in jsonl_changes:
        lines.append(f"- `{rel}`: {rows} rows, {count} ASCII dot leader page markers")
