...
- **复核时间：** 2026-07-06
- **当前文件：** `knowledge/herbs/nuodaogenxu.md`
- **队列位置：** `data/review_queue.jsonl` 第 88 行
- **条目：** 糯稻根须 (`nuodaogenxu`)
...
- herb_sources：status=`no_source_found`，source_hit_count=`0`，searched_keywords=['糯稻根须', '糯稻根', '稻根须', '稻根'].
...
- p30：classification=`external_source_required`，canonical_item_id=`nuodaogenxu`，risk_tier=`medium`。
- p36：category=`alias_first`，risk_tier=`medium`，recommended_source_scopes=['official_pharmacopoeia', 'modern_tcm_reference'].
- source FTS/LIKE：按名称 `糯稻根须` 检索得到 0 条 LIKE 命中；未检出可追溯命中。
