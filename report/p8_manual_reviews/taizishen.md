...
- **复核时间：** 2026-07-06
- **当前文件：** `knowledge/herbs/taizishen.md`
- **队列位置：** `data/review_queue.jsonl` 第 109 行
- **条目：** 太子参 (`taizishen`)
...
- herb_sources：status=`no_source_found`，source_hit_count=`0`，searched_keywords=['太子参'].
...
- source FTS/LIKE：按名称 `太子参` 检索得到 0 条 LIKE 命中；未检出可追溯命中。
