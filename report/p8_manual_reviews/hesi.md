...
- **队列位置：** `data/review_queue.jsonl` 第 51 行
- **条目：** 鹤虱 (`hesi`)
...
- p30：`internal_research_exhausted`；canonical=`hesi`；risk=`low`。
- p36：`internal_research_exhausted`；建议来源范围=['official_pharmacopoeia', 'modern_tcm_reference', 'classical_tcm_text']。
