...
- **复核时间：** 2026-07-06
- **当前文件：** `knowledge/herbs/chuanyubeimu.md`
- **队列位置：** `data/review_queue.jsonl` 第 25 行
- **条目：** 川贝母
...
- `review_queue`：第 25 行记录为 `no_source_found`。
...
- `source_fts.sqlite`：扩展命中为「贝母」通名，不等同「川贝母」专名；review_decisions 仍 no_source_found。
