...
- **复核时间：** 2026-07-06
- **当前文件：** `knowledge/herbs/tanxiang.md`
- **队列位置：** `data/review_queue.jsonl` 第 110 行
- **条目：** 檀香 (`tanxiang`)
...
- herb_sources：status=`no_source_found`，source_hit_count=`0`，searched_keywords=['檀香'].
...
- source FTS/LIKE：按名称 `檀香` 检索得到 0 条 LIKE 命中；未检出可追溯命中。
