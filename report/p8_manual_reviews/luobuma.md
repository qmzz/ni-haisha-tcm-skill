...
- **复核时间：** 2026-07-06
- **当前文件：** `knowledge/herbs/luobuma.md`
- **队列位置：** `data/review_queue.jsonl` 第 76 行
- **条目：** 罗布麻叶 (`luobuma`)
...
- p30：`alias_or_duplicate_mapped`；canonical=`luobumaye`；risk=`medium`。
- p36：当前分类=`alias_first`；建议来源范围以 p36 队列记录为准。
- 本条为别名/重复映射条目；规范条目仍无内部可追溯来源，故不据此提升正文质量。
