...
- **复核时间：** 2026-07-06
- **当前文件：** `knowledge/herbs/xiangyuan.md`
- **队列位置：** `data/review_queue.jsonl` 第 115 行
- **条目：** 香橼 (`xiangyuan`)
...
- herb_sources：status=`no_source_found`，source_hit_count=`0`，searched_keywords=['香橼'].
...
- source FTS/LIKE：按名称 `香橼` 检索得到 0 条 LIKE 命中；未检出可追溯命中。
