...
- **复核时间：** 2026-07-06
- **当前文件：** `knowledge/herbs/roudoukou.md`
- **队列位置：** `data/review_queue.jsonl` 第 96 行
- **条目：** 肉豆蔻 (`roudoukou`)
...
- herb_sources：status=`no_source_found`，source_hit_count=`0`，searched_keywords=['肉豆蔻'].
...
- source FTS/LIKE：按名称 `肉豆蔻` 检索得到 0 条 LIKE 命中；未检出可追溯命中。
