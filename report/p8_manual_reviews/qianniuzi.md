...
- **复核时间：** 2026-07-06
- **当前文件：** `knowledge/herbs/qianniuzi.md`
- **队列位置：** `data/review_queue.jsonl` 第 94 行
- **条目：** 牵牛子 (`qianniuzi`)
...
- herb_sources：status=`no_source_found`，source_hit_count=`0`，searched_keywords=['牵牛子', '黑丑', '白丑'].
...
- source FTS/LIKE：按名称 `牵牛子` 检索得到 0 条 LIKE 命中；未检出可追溯命中。
