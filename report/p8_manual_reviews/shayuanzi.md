...
- **复核时间：** 2026-07-06
- **当前文件：** `knowledge/herbs/shayuanzi.md`
- **队列位置：** `data/review_queue.jsonl` 第 102 行
- **条目：** 沙苑子 (`shayuanzi`)
...
- herb_sources：status=`no_source_found`，source_hit_count=`0`，searched_keywords=['沙苑子', '潼蒺藜'].
...
- source FTS/LIKE：按名称 `沙苑子` 检索得到 0 条 LIKE 命中；未检出可追溯命中。
