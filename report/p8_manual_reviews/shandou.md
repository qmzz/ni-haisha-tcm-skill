...
- **复核时间：** 2026-07-06
- **当前文件：** `knowledge/herbs/shandou.md`
- **队列位置：** `data/review_queue.jsonl` 第 99 行
- **条目：** 谷芽 (`shandou`)
...
- herb_sources：status=`no_source_found`，source_hit_count=`0`，searched_keywords=['谷芽'].
...
- p30：classification=`external_source_required`，canonical_item_id=`guya`，risk_tier=`medium`。
- p36：category=`alias_first`，risk_tier=`medium`，recommended_source_scopes=['official_pharmacopoeia', 'modern_tcm_reference'].
- source FTS/LIKE：按名称 `谷芽` 检索得到 0 条 LIKE 命中；未检出可追溯命中。
